...
public class Estado implements Serializable {
    ...
    public void loadEstado(String file) {
        ...
        for (String linha : linhas) {
            ...
            if (linhaPartida[0].equals("Casa")) {
                c = parseC(linhaPartida[1]);
                this.casas.add(c);
            } else if (linhaPartida[0].equals("Fornecedor")) {
                f = parseF(linhaPartida[1]);
                if (f != null) {
                    this.fornecedores.add(f.clone());
                }
            } else if (linhaPartida[0].equals("Divisao")) {
                divisao = linhaPartida[1];
                assert c != null;
                c.addRoom(divisao);
            } else if (linhaPartida[0].equals("SmartBulb")) {
                SmartBulb sb = parseSB(linhaPartida[1]);
                assert c != null;
                sb.setOn(true);
                c.addSmartDevice(sb);
                c.addToRoom(divisao, sb.getID());
            } else if (linhaPartida[0].equals("SmartSpeaker")) {
                SmartSpeaker ss = parseSS(linhaPartida[1]);
                assert c != null;
                ss.setOn(false);
                c.addSmartDevice(ss);
                c.addToRoom(divisao, ss.getID());
            } else if (linhaPartida[0].equals("SmartCamera")) {
    ...
    private Casa parseC(String linhaPartida){
        ...
        if (dados[2].equals("EDP")) {
            fornecedor = new FornecEDP();
        } else if (dados[2].equals("Endesa")) {
            fornecedor = new FornecEndesa();
        } else if (dados[2].equals("Jomar")) {
    ...
    private static Fornecedores parseF(String linhaPartida) {
        ...
        if (dados[0].equals("EDP")) {
            fornec = new FornecEDP();
        } else if (dados[0].equals("Endesa")) {
            fornec = new FornecEndesa();
        } else if (dados[0].equals("Jomar")) {
            fornec = new FornecJomar();
        } else {
            Menu.erros(15);
        }
        if (fornec != null) {
    ...
    private static SmartBulb parseSB(String linhaPartida) {
        ...
        if (dados[1].equals("Cold")) {
            sb.setTone(0);
        } else if (dados[1].equals("Warm")) {
            sb.setTone(2);
        } else if (dados[1].equals("Neutral")) {
            sb.setTone(1);
        } else {
            Menu.erros(8);
        }
